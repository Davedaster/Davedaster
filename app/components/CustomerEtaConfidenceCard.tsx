const BPD_LOGO_MARK_URL = 'https://cdn.shopify.com/s/files/1/0873/6250/2974/files/instagram_logo.png?v=1782327045';
...
function confidenceCopy({
  ...
  if (routeStatus !== 'OUT_FOR_DELIVERY') {
    return {
      eyebrow: 'Panel delivery planned',
      title: 'Your panel delivery slot',
      message: `We expect to be with you ${estimatedSlot}.`,
      highlight: 'We will update this page with live route progress once the driver starts their panel delivery route.',
    };
  }
  ...
  if (isNextDrop || deliveriesBefore === 0) {
    return {
      ...
      title: 'You are next',
      message: 'Your panel delivery is the next delivery on the route.',
  ...
  if (deliveriesBefore === 1) {
    return {
      ...
      title: '1 panel delivery before yours',
      message: 'There is just one panel delivery remaining before the driver heads to you.',
  ...
  return {
    ...
    title: `${deliveriesBefore} panel deliveries before yours`,
    message: 'Your panel delivery is progressing through the route and getting closer.',
    highlight: `Estimated arrival: ${estimatedSlot}.`,
  };
}
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
        <img
          src={BPD_LOGO_MARK_URL}
          alt='Bathroom Panels Direct'
          width={46}
          height={46}
      ...
      <p style={{ margin: '10px 0 0', color: '#667085', fontSize: 13 }}>
        Times are estimates and may change slightly due to traffic, access, or earlier panel deliveries.
      </p>
    </div>
  );
}
