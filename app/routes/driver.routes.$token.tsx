...
export const loader = async ({ params }: LoaderFunctionArgs) => {
  ...
  return json({ route, canStart: canStartDriverRoute(route.date) });
};
    ...
    <main style={{ minHeight: "100vh", background: "#f4f7fb", fontFamily: "Arial, sans-serif", color: "#323841" }}>
      <section style={{ maxWidth: 980, margin: "0 auto", padding: "20px 14px 36px" }}>
        <header style={{ background: "#ffffff", borderRadius: 18, padding: 18, boxShadow: "0 10px 28px rgba(50,56,65,0.12)", marginBottom: 14 }}>
          ...
          <p style={{ margin: "8px 0 0", color: "#667085" }}>Planned start: {formatStart(firstEta)}</p>
        ...
        <section style={{ background: "#ffffff", borderRadius: 18, padding: 14, boxShadow: "0 8px 24px rgba(50,56,65,0.08)", marginBottom: 14 }}>
          <RouteMap title="Driver route map" badge={`${mapPoints.length} pins`} points={mapPoints} height={340} />
        </section>

        <section style={{ background: "#ffffff", borderRadius: 18, padding: 14, boxShadow: "0 8px 24px rgba(50,56,65,0.08)", marginBottom: 14 }}>
          {actionData && "error" in actionData ? <p style={{ margin: "0 0 10px", color: "#b42318", fontWeight: 700 }}>{actionData.error}</p> : null}
          {routeStarted ? (
            ...
            <Form method="post">
              ...
              <button type="submit" disabled={!canStart} style={{ width: "100%", border: 0, borderRadius: 14, padding: "14px 16px", background: canStart ? "#509AE6" : "#d0d5dd", color: canStart ? "#ffffff" : "#667085", fontSize: 16, fontWeight: 700 }}>
                Start route
              </button>
              {!canStart ? <p style={{ margin: "10px 0 0", color: "#667085" }}>This route can only be started on {formatDate(route.date)}.</p> : null}
            </Form>
          )}
        </section>
            ...
            return (
              <article id={isNextStop ? "next-stop" : undefined} key={stop.id} style={{ background: isDelivered ? "#ecfdf3" : isFailed ? "#fef3f2" : "#ffffff", border: isDelivered ? "1px solid #86efac" : isFailed ? "1px solid #fecdca" : isNextStop ? "2px solid #509AE6" : "1px solid #e5e7eb", borderRadius: 18, padding: 16, boxShadow: "0 8px 24px rgba(50,56,65,0.08)" }}>
                ...
                <div style={{ marginTop: 14, display: "grid", gap: 9 }}>
                  ...
                  <div>
                    <strong>Items:</strong>
                    {lineItems.length ? <ul style={{ margin: "8px 0 0", paddingLeft: 20 }}>{lineItems.map((item, index) => <li key={`${item}-${index}`}>{item}</li>)}</ul> : <p style={{ margin: "6px 0 0", color: "#667085" }}>No item details stored for this order.</p>}
                  </div>
                </div>
                ...
                {!isDelivered && !isFailed ? (
                  <div style={{ marginTop: 14, display: "grid", gap: 12 }}>
                    <Form method="post" style={{ display: "grid", gap: 10 }}>
                      ...
                      <label style={{ display: "grid", gap: 6, fontWeight: 700 }}>Delivery note<textarea name="deliveryNote" rows={2} disabled={actionDisabled} style={{ border: "1px solid #d0d5dd", borderRadius: 12, padding: 10 }} /></label>
                      <label style={{ display: "grid", gap: 6, fontWeight: 700 }}>Safe place note<textarea name="safePlaceNote" rows={2} disabled={actionDisabled} style={{ border: "1px solid #d0d5dd", borderRadius: 12, padding: 10 }} /></label>
                      <label style={{ display: "flex", gap: 8, alignItems: "center" }}><input type="checkbox" name="leftInSafePlace" value="true" disabled={actionDisabled} />Left in safe place</label>
                    ...
                    <Form method="post" style={{ display: "grid", gap: 10 }}>
                      ...
                      <input type="hidden" name="stopId" value={stop.id} />
                      <label style={{ display: "grid", gap: 6, fontWeight: 700 }}>Missed delivery reason<input name="failedReason" disabled={actionDisabled} placeholder="No answer, access issue, customer unavailable" style={{ border: "1px solid #d0d5dd", borderRadius: 12, padding: 10 }} /></label>
                      <label style={{ display: "grid", gap: 6, fontWeight: 700 }}>Missed delivery note<textarea name="failedNote" rows={2} disabled={actionDisabled} style={{ border: "1px solid #d0d5dd", borderRadius: 12, padding: 10 }} /></label>
